...
module.exports = {
     ...
     */
    init: function (time, width = 200, height = 200, color = 'ffffff', bg = '000000', name = 'default', frames = 30, fontSize = 15, fontFamily = 'Courier New', cb) {
        ...
        fs.readdir(fontsFolder, (err, files) => {
            ...
            files.length && files.forEach(f => {
                registerFont(`${fontsFolder}/${f}`, {family: f.replace('.ttf', '')})
            })
        });
    ...
    encode: function (timeResult, cb) {
        ...
        let fontSize = Math.floor(this.fontSize) + 'px';
        let fontweight = 15 + 'px';

        // set the font style
        ctx.font = [fontSize, `"${this.fontFamily}"`, fontweight].join(' ');
